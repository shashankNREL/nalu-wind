...
void GammaEquationSystem::register_interior_algorithm(stk::mesh::Part *part)
{
  ...
  if ( it == assembleNodalGradAlgDriver_->algMap_.end() ) {
    ...
    if ( edgeNodalGradient_ && realm_.realmUsesEdges_ ) {
      theAlg = new AssembleNodalGradEdgeAlgorithm(realm_, part, &gammaNp1, &dGamdxNone);
    }
    else {
  ...
  if (!realm_.solutionOptions_->useConsolidatedSolverAlg_) {
    ...
    if (itsi == solverAlgDriver_->solverAlgMap_.end()) {
      ...
      if (realm_.realmUsesEdges_) {
        theAlg = new ScalarEdgeSolverAlg(realm_, part, this, gamma_, dGamdx_, evisc_);
      }
      else {
        throw std::runtime_error("Gamma transition model not implemented for Element-based scheme");
      }
    ...
    const InflowBoundaryConditionData &inflowBCData)
{
  ...
  ScalarFieldType &gammaNp1 = sdr_->field_of_state(stk::mesh::StateNP1);
  VectorFieldType &dGamdxNone = dwdx_->field_of_state(stk::mesh::StateNone);
  ...
  if ( it == assembleNodalGradAlgDriver_->algMap_.end() ) {
    Algorithm *theAlg 
      = new AssembleNodalGradBoundaryAlgorithm(realm_, part, &gammaNp1, &dGamdxNone, edgeNodalGradient_);
    assembleNodalGradAlgDriver_->algMap_[algType] = theAlg;
  }
  else {
    it->second->partVec_.push_back(part);
